...
export default function MercurIAHomePage() {
  ...
  const [primaryColor, setPrimaryColor] = useState('#0891B2'); // Ciano como cor primária padrão
    ...
    <div className={`min-h-screen ${themeColors.background} ${themeColors.textPrimary} p-4 sm:p-6 lg:p-8 flex flex-col items-center font-sans`}>
      ...
      <main className={`w-full max-w-3xl ${themeColors.cardBackground} p-6 sm:p-8 rounded-xl shadow-2xl space-y-8 border ${themeColors.borderDefault}`}>
        ...
        {currentStep === 'initialInput' && (
          <>
            <div className="space-y-8"> {/* Aumentado o espaçamento entre secções */}
                <section>
                    ...
                    <div className="mb-5">
                        ...
                        <div className="flex space-x-4">
                            <button onClick={() => {setContentInputMode('text'); setBaseContentProcessed(false);}} className={`flex-1 py-2.5 px-5 rounded-lg transition-all duration-150 ease-in-out focus:outline-none ${themeColors.focusRing} focus:ring-offset-2 text-sm font-medium ${contentInputMode === 'text' ? `${themeColors.buttonPrimaryBg} ${themeColors.buttonPrimaryText} shadow-lg transform hover:scale-105` : `bg-white ${themeColors.textPrimary} border ${themeColors.borderInput} hover:bg-slate-50`}`}>Digitar Texto</button>
                            <button onClick={() => {setContentInputMode('file'); setBaseContentProcessed(false);}} className={`flex-1 py-2.5 px-5 rounded-lg transition-all duration-150 ease-in-out focus:outline-none ${themeColors.focusRing} focus:ring-offset-2 text-sm font-medium ${contentInputMode === 'file' ? `${themeColors.buttonPrimaryBg} ${themeColors.buttonPrimaryText} shadow-lg transform hover:scale-105` : `bg-white ${themeColors.textPrimary} border ${themeColors.borderInput} hover:bg-slate-50`}`}>Carregar Ficheiro</button>
                        </div>
                    </div>
                ...
                <section>
                  <h2 className={`text-2xl sm:text-3xl font-semibold ${themeColors.textPrimary} mb-2`}>Passo 2: Personalize a Apresentação</h2>
            ...
            {!baseContentProcessed && (<button onClick={handleProcessBaseContent} disabled={isLoading} className={`w-full mt-8 ${themeColors.buttonPrimaryBg} ${themeColors.buttonPrimaryText} font-semibold py-3 px-4 rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-150 flex items-center justify-center text-lg disabled:opacity-70 disabled:cursor-not-allowed focus:outline-none ${themeColors.focusRing} focus:ring-offset-2`}><Icon name="wand" className="w-5 h-5 mr-2.5" />Analisar e Definir Opções IA</button>)}
            {baseContentProcessed && (
                <section className="animate-fadeIn space-y-8 mt-8 pt-8 border-t border-slate-200">
                    <div>
                        <h2 className={`text-2xl sm:text-3xl font-semibold ${themeColors.textPrimary} mb-2`}>Passo 3: Assistência Criativa da IA</h2>
                        <p className={`${themeColors.textSecondary} text-sm mb-5`}>Selecione como a IA pode ajudar. Estas opções serão aplicadas ao gerar o rascunho.</p>
                        {initialProcessedContent && <details className={`mb-5 bg-white p-3.5 rounded-lg text-xs border ${themeColors.borderDefault} shadow-sm`}><summary className={`cursor-pointer ${themeColors.textSecondary} hover:${themeColors.textPrimary} flex items-center font-medium`}><Icon name="info" className="w-4 h-4 mr-1.5 text-slate-400"/>Ver resumo do conteúdo base</summary><pre className={`whitespace-pre-wrap mt-2.5 max-h-36 overflow-y-auto ${themeColors.textSecondary} p-2 bg-slate-50 rounded`}>{initialProcessedContent}</pre></details>}
        ...
        {currentStep === 'approveContent' && ( 
            <section className="animate-fadeIn">
                <h2 className={`text-2xl sm:text-3xl font-semibold ${themeColors.textPrimary} mb-2`}>Passo 5: Reveja e Aprove o Rascunho</h2>
                ...
                <ul className="text-xs list-disc list-inside mb-5 pl-4 text-slate-500">
                ...
                <div className={`p-4 bg-white rounded-lg max-h-[400px] overflow-y-auto border ${themeColors.borderInput} mb-6 shadow-inner`}><pre className={`whitespace-pre-wrap ${themeColors.textPrimary} text-sm leading-relaxed`}>{contentForApproval || "Nenhum conteúdo gerado."}</pre></div>
        ...
        {currentStep === 'finalResult' && ( 
            <section className={`animate-fadeIn`}>
                ...
                {finalResult && (
                    <>
                        <div className={`space-y-2 ${themeColors.textSecondary} mb-6 p-5 border ${themeColors.borderDefault} rounded-lg bg-white shadow-sm`}>
                        ...
                        <details className={`mb-5 bg-white p-3.5 rounded-lg text-xs border ${themeColors.borderDefault} shadow-sm`}><summary className={`cursor-pointer ${themeColors.textSecondary} hover:${themeColors.textPrimary} flex items-center font-medium`}><Icon name="info" className="w-4 h-4 mr-1.5 text-slate-400"/>Ver conteúdo aprovado (simulação)</summary><pre className={`whitespace-pre-wrap mt-2.5 max-h-40 overflow-y-auto ${themeColors.textSecondary} p-2 bg-slate-50 rounded`}>{finalResult.approvedContent}</pre></details>
      ...
      <style jsx global>{`
        ...
        textarea::-webkit-scrollbar-track, .max-h-32::-webkit-scrollbar-track, .max-h-40::-webkit-scrollbar-track, .max-h-\\[400px\\]::-webkit-scrollbar-track { background: #E2E8F0; border-radius: 10px; } /* Tailwind slate-200 */
        textarea::-webkit-scrollbar-thumb, .max-h-32::-webkit-scrollbar-thumb, .max-h-40::-webkit-scrollbar-thumb, .max-h-\\[400px\\]::-webkit-scrollbar-thumb { background: #94A3B8; border-radius: 10px; } /* Tailwind slate-400 */
        textarea::-webkit-scrollbar-thumb:hover, .max-h-32::-webkit-scrollbar-thumb:hover, .max-h-40::-webkit-scrollbar-thumb:hover, .max-h-\\[400px\\]::-webkit-scrollbar-thumb:hover { background: #64748B; } /* Tailwind slate-500 */
